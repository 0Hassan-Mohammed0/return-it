...
class _OtpVerificationScreenState extends State<OtpVerificationScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (args != null && _email == null) {
      // Check _email==null to avoid re-running on rebuilds
      _email = args['email'];
      _isRegistration = args['isRegistration'] ?? false;
      _registrationData =
          args['userData']; // Arguments map might just merge user data?
  ...
  Future<void> _startResend({bool silent = false}) async {
    ...
    bool success = await _emailAuth.sendOTP();
    if (mounted && !silent) {
      if (success) {
  ...
  void _verifyOtp() async {
    ...
    if (otp.length < 4) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please enter valid code')));
      return;
    }
    ...
    if (verified) {
      ...
      if (_isRegistration && _registrationData != null) {
        try {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Account Verified & Created!')));
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Too many failed attempts. Try again in $hoursLeft hours.',
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintText: "------",
                    hintStyle:
                        TextStyle(color: Colors.grey[300], letterSpacing: 16),
                    filled: true,
